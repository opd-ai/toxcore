...
- [x] low documentation — doc.go:28 correctly uses `LastSeenDuration()` — no action needed (`doc.go:28`, `friend.go:269`)
...
4. ~~Update doc.go example to use LastSeenDuration() instead of non-existent GetLastSeen()~~ — **N/A**: doc.go already correctly uses `LastSeenDuration()` at line 28
