...
## Summary
The friend package implements core Tox friend management including Friend struct, friend requests, and request handling. While the basic structure is sound with good test coverage for core Friend methods, the package has critical issues: non-deterministic timestamp usage (time.Now()), duplicate Friend type definition with toxcore.go causing maintenance issues, missing concurrency protection in RequestManager, and no serialization support for persistence. Integration with the main Tox instance is incomplete.

## Issues Found
- [ ] high — **Duplicate Friend type definition** — Friend struct defined both in `friend/friend.go` and `toxcore.go:1736`, causing type conflicts and maintenance burden; the toxcore.go version is used in actual Tox implementation while friend/ version is isolated (`friend.go:41`, `toxcore.go:1736`)
- [ ] high — **Missing concurrency protection** — RequestManager has no mutex protection for pendingRequests slice accessed from multiple methods, risking race conditions (`request.go:102-186`)
- [ ] high — **Non-deterministic timestamp usage** — Uses `time.Now()` directly in multiple locations, violating deterministic procgen requirement for reproducible state (`friend.go:64`, `friend.go:146`, `request.go:38`, `request.go:91`)
...
- The friend/ package defines a Friend type that is NOT used by the main Tox implementation. Instead, toxcore.go:1736 defines its own Friend type with slightly different fields (adds IsTyping field, uses FriendStatus instead of Status enum).
...
- No serialization support means Friend/Request state cannot be persisted in savedata.
...
- Friend state should serialize/deserialize for savedata support
- Connection status changes should trigger DHT routing updates

## Recommendations
1. **CRITICAL**: Resolve Friend type duplication - either consolidate into single definition or rename friend.Friend to avoid conflicts with toxcore.Friend; current state causes type incompatibility
2. **CRITICAL**: Add sync.RWMutex to RequestManager to protect pendingRequests slice from concurrent access in multi-goroutine environments
3. **HIGH**: Replace all time.Now() calls with injectable time provider pattern (clock interface) to enable deterministic testing and state reproducibility
4. **HIGH**: Implement Marshal/Unmarshal methods for Friend and Request types to support savedata persistence
