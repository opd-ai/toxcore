...
### Low Priority
~~1. **Document Privacy Network Status More Clearly** - Consider adding a "Roadmap" section to clarify which features are complete vs planned.~~ **COMPLETED 2026-01-28**: Added comprehensive Roadmap section to README.md (after line 1082) documenting feature status with three categories: "✅ Fully Implemented Features", "🚧 Planned Features", and "📋 Future Considerations". The Roadmap clearly documents that privacy networks (Tor/I2P/Nym/Lokinet) have interface support but no transport implementation, and that LocalDiscovery option exists but is not implemented. Includes migration notes for future compatibility.

~~2. **Implement Local Discovery** - Either implement the feature or mark it as "reserved for future implementation" in documentation.~~ **COMPLETED 2026-01-28**: Documented in the new Roadmap section under "🚧 Planned Features" as "Local Network Discovery (Reserved for Future Implementation)". Clearly states that the `LocalDiscovery` option exists but has no implementation, and explains it's low priority since bootstrap nodes provide adequate peer discovery. Users are informed of the current status and that the feature is planned for future development.

~~3. **Review Friend Status Notification Flow** - Ensure all paths that update friend connection status notify the async manager.~~ **COMPLETED 2026-01-28**: Implemented `SetFriendConnectionStatus()` method in toxcore.go that properly updates friend connection status and notifies the async manager when online status changes. Added `GetFriendConnectionStatus()` for retrieval. The implementation ensures `updateFriendOnlineStatus()` is called whenever connection status transitions between offline (ConnectionNone) and online (ConnectionUDP/ConnectionTCP), enabling the async manager to trigger pre-key exchanges for forward-secure messaging. Created comprehensive test suite in friend_connection_status_test.go with 6 test functions covering notification flow, edge cases, and integration scenarios. All tests pass.
