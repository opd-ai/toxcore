...
### High Severity
- [x] **[CRITICAL]** Version negotiation packets are unauthenticated — an active attacker can strip `PacketVersionNegotiation` (type 249) to force downgrade to legacy protocol when `fallbackEnabled=true` (`transport/negotiating_transport.go:110-121`)
