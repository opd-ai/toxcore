...
// See M-02 in AUDIT.md: a forged ciphertext can still consume a pre-key. The fix
// requires additional refactoring to reserve (but not persist) the key before
// decrypt, then commit only on success. For now, this deprecated path uses the
// existing behavior; the recommended obfuscated path is not affected.
